...
parser.add_argument('--pre-epochs', default=100, type=int, metavar='N',
...
parser.add_argument('--start-label', default=0, type=float,
                        help='inital OOD score of unlabeled data')
#MixMatch options
...
#OOD options
parser.add_argument('--outdata', type=str, default='LSUN_resize',
...
def main():
    ...
    train_labeled_set, train_unlabeled_set, train_domain_set, val_set, test_set = dataset.get_cifar10('./data', args.n_labeled, args.outdata, transform_train=transform_train, transform_val=transform_val)

    domain_trainloader = data.DataLoader(train_domain_set, batch_size=args.batch_size, shuffle=True, num_workers=0)
    ...
    def create_model(ema=False):
        model = models.WideResNet2(num_classes=10)
        model = model.cuda()
    ...
    if args.resume:
        ...
        logger.set_names(['Train Loss', 'Train Loss X', 'Train Loss U',  'Valid Loss', 'Valid Acc.', 'Test Loss', 'Test Acc.', 'Domain Prec.', 'Domain Recall.'])
    ...
    if not args.resume:
        for epoch in range(args.pre_epochs):
            ...
            train_loss, train_loss_x, train_loss_u = domain_train(domain_trainloader, model, optimizer, ema_optimizer, train_criterion, epoch, use_cuda)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loss, train_loss_x, train_loss_u, prec, recall = train(domain_trainloader, labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, train_criterion, epoch, use_cuda)
        ...
        # append logger file
        logger.append([train_loss, train_loss_x, train_loss_u, val_loss, val_acc, test_loss, test_acc, prec, recall])
...
def domain_train(domain_trainloader, model, optimizer, ema_optimizer, criterion, epoch, use_cuda):
    ...
    model.train()
    for batch_idx, (inputs, targets, domain_labels, indexs) in enumerate(domain_trainloader):
...
def train(domain_trainloader, labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion, epoch, use_cuda):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        # plot progress
        bar.suffix  = '({batch}/{size}) Data: {data:.3f}s | Batch: {bt:.3f}s | Total: {total:} | ETA: {eta:} | Loss: {loss:.4f}| W: {w:.4f} | TH: {th:.4f} | N:{n:.4f} | Prec:{prec:.4f}| Recall:{recall:.4f}'.format(
                    batch=batch_idx + 1,
    ...
    return (losses.avg, losses_x.avg, losses_u.avg, prec, recall)
